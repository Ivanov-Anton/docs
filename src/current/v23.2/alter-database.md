...
In this example, we use [Zone Config Extensions]({% link {{ page.version.version }}/zone-config-extensions.md %}) to provide an alternative implementation of [Super Regions]({% link {{ page.version.version }}/multiregion-overview.md %}#super-regions), which are useful for [data domiciling]({% link {{ page.version.version }}/data-domiciling.md %}).
...
Next, enter the statements below at the SQL prompt. These statements will:

- Make the [`movr` database]({% link {{ page.version.version }}/movr.md %}) into a [multiregion database]({% link {{ page.version.version }}/multiregion-overview.md %}) by [setting the primary region]({% link {{ page.version.version }}/alter-database.md %}#set-primary-region).
...
- Add the remaining regions we defined at cluster start time to the multi-region `movr` database (for a total of 6).
...
Now that our multiregion cluster is configured, we will use the `ALTER LOCALITY` statements shown below to [configure replication zones]({% link {{ page.version.version }}/configure-replication-zones.md %}) such that:
...
In this example, we use [Zone Config Extensions]({% link {{ page.version.version }}/zone-config-extensions.md %}) to minimize cross-region write latency in a cluster with the following characteristics:

- 3 main US regions (`us-west1`,`us-east1`,`us-central1`).
- 2 additional distant regions (`europe-west1`, `asia-northeast1`). Only [follower reads]({% link {{ page.version.version }}/follower-reads.md %}) will be made from these regions.

    To minimize the cross-region write latency, we will [configure replication zones]({% link {{ page.version.version }}/configure-replication-zones.md %}) such that the 3 US regions have voting replicas (that is, replicas that participate in [Raft quorum]({% link {{ page.version.version }}/architecture/replication-layer.md %}#raft)). This will leave the 2 additional non-US regions with only [non-voting replicas]({% link {{ page.version.version }}/architecture/replication-layer.md %}#non-voting-replicas).
...
- Make the [`movr` database]({% link {{ page.version.version }}/movr.md %}) into a [multiregion database]({% link {{ page.version.version }}/multiregion-overview.md %}) by [setting the primary region]({% link {{ page.version.version }}/alter-database.md %}#set-primary-region).
- Set the `movr` database to [survive region failures]({% link {{ page.version.version }}/multiregion-survival-goals.md %}#survive-region-failures).
- Add the remaining regions we defined at cluster start time to the multi-region `movr` database (for a total of 5).
...
- This will leave the 2 additional regions (Europe and Japan) with only [non-voting replicas]({% link {{ page.version.version }}/architecture/replication-layer.md %}#non-voting-replicas) that can be used to serve [follower reads]({% link {{ page.version.version }}/follower-reads.md %}).
